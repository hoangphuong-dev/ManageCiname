    ...
    <template #main>
      <div class="bg-white min-h-full m-4 mb-0 p-4">
        ...
        <div class="w-full flex relative">
          <div class="w-3/4 flex items-end">
            <div class="mr-2">
              <div class="text-sm text-blackPrimary-300">送信対象</div>
              <el-select
        ...
        <div class="mt-5">
          ...
          >
            ...
            <template #receiver="{ row }">
              <div class="receiver-parent" v-html="renderReceiver(row)"></div>
            </template>
            <template #actions="{ row }">
              <div v-if="row" class="flex items-center">
                <template v-if="!row.status">
                  <button class="btn-warning bg-gray-200" @click="edit(row)">
                    <img src="/images/svg/edit.svg" class="size-icon" alt="" />
                  </button>
                  &nbsp;
                </template>
                <el-popconfirm
                ...
                &nbsp;
                <button class="btn-warning" @click="detail(row)">詳細</button>
              </div>
            </template>
          </data-table>
        </div>
      ...
      <div class="dialog-form-notice">
        <el-dialog
          ...
          width="60%"
          :title="detailSelected.title + ' - ' + detailSelected.schedule"
        >
          <div v-html="'Delivery To: ' + generateReceiverDetail()"></div>
          <hr />
          <div
...
export default {
  ...
  methods: {
    ...
    onChangeAllHospital(val) {
      if (val) {
        this.formData.receiver_ids = [...this.getUserSelectedByRole(ROLE_USER)];
      }
    },
    ...
    renderReceiver(row) {
      if (typeof row === "undefined" || Object.entries(row).length === 0) {
        return;
      }
      ...
      return `<span class="receiver-count ${
        notificationManager.length <= 5 ? "receiver-green" : "receiver-orange"
    ...
    generateReceiverDetail() {
      const notificationManager = this.detailSelected.notification_manager;
      ...
      for (const [index, item] of Object.entries(notificationManager)) {
        ...
        xhtml += parseInt(index) !== notificationManager.length - 1 ? ", " : "";
      }
      return xhtml;
    },
