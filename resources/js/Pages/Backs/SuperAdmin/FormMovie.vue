    ...
    methods: {
        ...
        async onSubmit() {
            ...
            this.$refs["formMovie"].validate(async (valid) => {
                if (valid) {
                    Inertia.post(route("superadmin.movie.store"), {
                        ...this.formData,
                    });
                }
            });
            ...
            this.$refs["movieGenreForm"].validate(async (valid) => {
                if (valid) {
                    await createMovieGenre(
                        toFormData({ ...this.formDataMoviegenre }, "", {
                            indices: true,
                        })
                    )
                        .then(async (res) => {
        ...
        async submitFormat() {
            this.$refs["FormatForm"].validate(async (valid) => {
                if (valid) {
                    await createFormat(
                        toFormData({ ...this.formDataFormat }, "", {
                            indices: true,
                        })
                    )
                        .then(async (res) => {
        ...
        async fetchDataMovieGenre() {
            ...
            listMovieGenre()
                .then(({ status, data }) => {
                    this.movie_genres =
                        status === 200 ? data.data : this.movie_genres;
                })
                .catch(() => {});
            this.loading = false;
        },
