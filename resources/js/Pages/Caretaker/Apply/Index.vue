  ...
  <app-layout title="Dashboard">
    <div class="py-12">
      <div class="mx-auto messenger-window__custom">
        <el-row :gutter="10" class="mb-8">
          <el-col :md="{ span: 6 }">
            <div class="flex items-center">
              <el-input class="h-10" :placeholder="'Search'" v-model="search"></el-input>

              <button class="btn-primary h-10 -ml-1 z-10" @click.prevent="onSearch()">
        ...
        <div class="w-full">
          <div class="main-jobs">
            <el-tabs v-model="activeName" @tab-click="handleClick" class="tab-custom">
              <el-tab-pane
  ...
  setup(props) {
    ...
    const mapJob = jobs.data.map((item) => {
      ...
      if (Array.isArray(job_apply)) {
        job_apply = item.job_apply.find((i) => i.times === item.times);
      }

      return {
        ...item,
        job_apply,
        last_update: job_apply?.job_post_apply?.last()?.updated_at || "",
      };
    });
    ...
    const getRoute = (page) => {
      const routeName =
        activeName.value === "first"
          ? "caretaker.get.job.apply"
          : "caretaker.get.job.applied";
