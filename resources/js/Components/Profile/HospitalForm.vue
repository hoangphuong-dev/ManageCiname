  ...
  <section class="HospitalForm my-8 bg-white px-10 py-6">
    ...
    >
      ...
      <div class="flex w-full mt-5 gap-4">
        ...
        <div class="w-1/4">
          ...
          >
            <el-select
              class="w-full"
              v-model="formData.hospital_type_id"
              placeholder="Select"
            >
              <el-option
      ...
      <div class="my-10 flex gap-4">
        <button class="btn-info min-w-132" @click.prevent="onSubmit">
          キャンセル
        </button>
        <button class="btn-warning min-w-132" @click.prevent="onSubmit">
          登録
        </button>
      </div>
    </el-form>
...
export default {
  ...
  methods: {
    initData() {
      ...
      this.formData.hospital_type_id =
        this.hospital?.hospital_info?.hospital_type_id;
    ...
    async onSubmit() {
      this.$refs["hospitalFormRef"].validate(async (valid) => {
        if (valid) {
          ...
          await updateProfile(
            toFormData({ ...this.formData, _method: "PUT" }, "", {
              indices: true,
            })
          )
            .then(async (res) => {
