  ...
  <div class="main-jobs__item">
    <div class="main-jobs__title">
      <div
        ...
        @click="$inertia.get(route('caretaker.job_detail', job?.id))"
      >
        {{ job.name }}
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2">
        <div>
          ...
          <div
            ...
            :style="{
              'border-top':
                (jobItem.tags || []).length === 0 ? '1px dashed #979797' : null,
            }"
          >
            <div
              class="
          ...
          <div class="flex">
            ...
            <div class="main-jobs__list-content">
              {{ jobItem.annual_income_min }}万円～{{
                jobItem.annual_income_max
              }}万円程度
            </div>
          </div>
        ...
        <div v-for="(comment, index) in comments" :key="index">
          ...
          <div class="flex items-center text-blackPrimary-300">
            <div class="w-2/3 flex items-center">
              ...
              <div class="font-bold text-blackPrimary-500 text-xl mx-5">
                {{ comment?.user?.name }}
              </div>
              <div class="py-1.5 px-2 bg-grayPrimary rounded">
                ID: {{ comment?.user?.id }}
              </div>
            </div>
            <div class="w-1/3 text-right">
              レビュー日時: {{ formatDateTime(comment.created_at) }}
            </div>
          </div>
