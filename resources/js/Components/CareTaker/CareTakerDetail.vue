<template>
    <section class="CareTakerInfo flex my-8">
        <div class="w-3/4 bg-white px-6 py-7">
            <div class="flex mb-5">
                <div class="w-1/2 flex items-center text-yellowPrimary">
                    ...
                    <div class="rounded p-1 mr-2 bg-grayPrimary">ID:{{ caretaker.id }}</div>
                    <div class="rounded p-1 mr-2 bg-grayPrimary">{{ caretakerData?.user_info?.age }}歳</div>
                    <div class="rounded p-1 bg-grayPrimary">{{ caretakerData?.user_info?.gender }}</div>
                </div>
            ...
            <div class="mt-4 border border-grayBorder">
                <div class="flex flex-wrap min-h-64 border-b-gray">
                    ...
                    <div class="w-4/5 flex px-3 items-center">{{ caretakerData?.name }}</div>
                </div>
                <div class="flex flex-wrap min-h-64 border-b-gray">
                    <h3 class="w-1/5 px-3 bg-grayPrimary flex items-center">生年月日</h3>
                    <div class="w-4/5 flex px-3 items-center">{{ caretakerData?.user_info?.birthday }}</div>
                </div>
                <div class="flex flex-wrap min-h-64 border-b-gray">
                    <h3 class="w-1/5 px-3 bg-grayPrimary flex items-center">現住所</h3>
                    <div class="w-4/5 flex px-3 items-center">{{ caretakerData?.user_info?.address }}</div>
                </div>
                <div class="flex flex-wrap min-h-64 border-b-gray">
                    <h3 class="w-1/5 px-3 bg-grayPrimary flex items-center">現住所以外に連絡を希望する場合の住所</h3>
                    <div class="w-4/5 flex px-3 items-center">{{ caretakerData?.user_info?.address_other }}</div>
                </div>
                <div class="flex flex-wrap min-h-64 border-b-gray">
                    <h3 class="w-1/5 px-3 bg-grayPrimary flex items-center">学歴</h3>
                    <div class="w-4/5 flex px-3 items-center">
                        <span v-for="(education, index) in caretakerData?.educations" :key="index">
                            {{ education.name }}
                        </span>
                    </div>
                </div>
                <div class="flex flex-wrap min-h-64 border-b-gray">
                    ...
                    <div class="w-4/5 flex px-3 items-center">
                        <span v-for="(experience, index) in caretakerData?.experiences" :key="index">
                            {{ experience.name }}
                        </span>
                    </div>
                </div>
                <div class="flex flex-wrap min-h-64 border-b-gray">
                    <h3 class="w-1/5 px-3 bg-grayPrimary flex items-center">免許・資格（取得予定資格を含む）</h3>
                    <div class="w-4/5 flex px-3 items-center">
                        <span v-for="(qualification, index) in caretakerData?.qualifications" :key="index">
                            {{ qualification.name }}
                        </span>
                    </div>
                </div>
                <div class="flex flex-wrap min-h-64 border-b-gray">
                    <h3 class="w-1/5 px-3 bg-grayPrimary flex items-center">健康状態※選択式</h3>
                    <div class="w-4/5 flex px-3 items-center">{{ caretakerData?.user_info?.health_detail }}</div>
                </div>
            </div>
            ...
            <div class="timeline mt-10">
                ...
                <div v-for="(item, i) in processes" :key="i" class="flex timeline-step">
                    ...
                    <div class="text-blackPrimary-500 mb-6">
                        ...
                        <div class="flex">
                            <div class="w-52 rounded py-2 px-4" :class="getClassProcess(item.id)">
                                {{ item.content }}
                            </div>
                            <template v-if="getStatusProcess(item.id) === 0">
                                <button class="ml-5 btn-success" @click="onDoneProcess(item.id)">合格</button>
                                <button class="ml-2 btn-warning" @click="onRejectProcess(item.id)">不合格</button>
                            </template>
                        </div>
                    </div>
                </div>
        ...
        <div class="w-1/4 ml-5 bg-white p-4">
            ...
            <div v-for="(item, i) in [1, 2, 3, 4, 5]" :key="i" class="candidate-related">
                <div class="flex">
                    <div class="w-24">
                        <img class="h-full w-full" src="https://via.placeholder.com/96x96" alt="" />
                    </div>
                    <div class="ml-4">
                        <div class="text-2xl text-blackPrimary-500 mb-2">東龍太郎</div>
                        <div>
                            <span class="rounded-2xl p-1 mr-2 bg-grayPrimary text-sm">男性</span>
